$(document).ready(function () {
    let total = 100; // 数据总条数
    let PageIndex = 1;// 当前页
    let PageSize = 5; //一页显示10条
    let pageHtml = $('.pagination'); // 分页栏
    let totalPage = Math.ceil(total / PageSize); // 获取到总页数
    function setPages() {
        //上一页
        let pageBtnPrev = "<li class='page-item btnPrev'><a class='page-link' aria-label='Previous' href='/book/getBookAll?${pagesList.getPrePage}'><span aria-hidden='true'>«</span></a ></li>";
        //下一页
        let pageBtnNext = "<li class='page-item btnNext'><a class='page-link' aria-label='Next'     href='/book/getBookAll?${pagesList.getNextPage}'><span aria-hidden='true'>»</span></a></li>";
        let pageTips = "<div style='line-height:38px'>" + PageIndex + " " + "-" + " " + totalPage + " " + "of" + " " + total + " " + "items" + "</div > ";
        pageHtml.append(pageBtnPrev); // 添加左按钮
        let homePage = $("<li class='page-item' pagenumber=1><a class='page-link' href='/book/getBookAll?currentPage=1' >首页</a ></li> ");
        // 第一页
        ...
        //尾页
        let lastPage1 = $("<li class='page-item' pagenumber=1><a class='page-link' href='/book/getBookAll?${pagesList.getTotalPage}' >首页</a ></li> ");
        // 总页数小于等于10页全部显示
        if (totalPage <= 5) {
